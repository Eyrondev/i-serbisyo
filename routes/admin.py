...
from activity_logger import log_resident_action, ActivityTypes
...
# Add context processor to inject notification counts into all admin templates
...
def dashboard():
    ...
    # Calculate monthly revenue from paid certificates and payments
    current_month = datetime.now().month
    current_year = datetime.now().year
    
    # Revenue from Payment table ONLY (to avoid double counting)
    # Payment table contains all certificate payments that have been completed and paid
    payment_revenue = db.session.query(func.sum(Payment.amount)).filter(
    ...
    # Debug logging
    print(f"DEBUG - Dashboard Monthly Revenue:")
    print(f"  Current Month: {current_month}, Year: {current_year}")
    print(f"  Payment Revenue: ₱{payment_revenue}")
    print(f"  Total Monthly Revenue: ₱{monthly_revenue}")
    ...
    # Revenue from Payment table ONLY (to avoid double counting)
    last_month_payment_revenue = db.session.query(func.sum(Payment.amount)).filter(
        Payment.payment_status == 'paid',
        Payment.payment_date.isnot(None),
        extract('month', Payment.payment_date) == last_month,
        extract('year', Payment.payment_date) == last_year
    ).scalar() or Decimal('0.00')
    
    last_monthly_revenue = last_month_payment_revenue
    
    print(f"  Last Month ({last_month}/{last_year}) Revenue: ₱{last_monthly_revenue}")
    
    # Calculate revenue change percentage
    revenue_change = 0
    if last_monthly_revenue > 0:
        revenue_change = round(((monthly_revenue - last_monthly_revenue) / last_monthly_revenue) * 100, 1)
    elif monthly_revenue > 0:
        # If last month was 0 but this month has revenue, show as 100% increase
        revenue_change = 100
    ...
    # Calculate percentage change for requests (compare current month to previous month)
    ...
    weekly_announcements = Announcement.query.filter(Announcement.created_at >= week_ago).count()
    
    stats = {
        'total_residents': total_residents,
        'pending_requests': pending_requests + pending_residents,
        ...
        'weekly_announcements': weekly_announcements
    }
...
@admin_bp.route('/pending-residents')
@admin_required
def pending_residents():
    ...
    if date_from:
        try:
            ...
            query = query.filter(db.func.date(Resident.created_at) >= from_date)
        except ValueError:
            pass
    ...
    if date_to:
        try:
            ...
            query = query.filter(db.func.date(Resident.created_at) <= to_date)
        except ValueError:
            pass
    ...
    return render_template('admin/pending-residents.html', 
                         residents=residents,
                         stats=stats,
...
def officials():
    ...
    return render_template('admin/officials.html', 
...
@admin_bp.route('/api/announcement/<int:announcement_id>', methods=['GET'])
@admin_required
...
@admin_bp.route('/api/announcement/<int:announcement_id>', methods=['PUT'])
@admin_required
...
@admin_bp.route('/api/announcement/<int:announcement_id>', methods=['DELETE'])
@admin_required
...
@admin_bp.route('/api/announcement/<int:announcement_id>/publish', methods=['POST'])
@admin_required
...
@admin_bp.route('/api/announcement/<int:announcement_id>/unpublish', methods=['POST'])
@admin_required
...
@admin_bp.route('/api/payment-list/<int:payment_id>')
@admin_required
...
@admin_bp.route('/api/payment-list', methods=['POST'])
@admin_required
...
@admin_bp.route('/api/payment-list/<int:payment_id>', methods=['PUT'])
@admin_required
...
@admin_bp.route('/api/payments/<int:payment_id>', methods=['DELETE'])
@admin_required
...
@admin_bp.route('/api/payment-list/<int:payment_id>/mark-paid', methods=['POST'])
@admin_required
...
def activity_log():
    ...
    return render_template('admin/activity-log.html', 
...
@admin_bp.route('/api/approve-resident/<int:resident_id>', methods=['POST'])
@admin_required
...
@admin_bp.route('/api/resident-details/<int:resident_id>', methods=['GET'])
@admin_required
...
def reject_resident_with_reason(resident_id):
    try:
        ...
        if resident.email:
            try:
                send_rejection_email_admin(resident, rejection_reason)
...
def send_rejection_email_admin(resident, rejection_reason):
...
@admin_bp.route('/api/add-official', methods=['POST'])
@admin_required
def add_official():
    try:
...
@admin_bp.route('/api/update-official/<int:official_id>', methods=['POST'])
@admin_required
def update_official(official_id):
    try:
        ...
        # Handle form data instead of JSON for file uploads
        name = request.form.get('name', official.name)
        position = request.form.get('position', official.position)
        committee = request.form.get('committee', official.committee)
        email = request.form.get('email', official.email)
        phone = request.form.get('phone', official.phone)
        term_start = request.form.get('term_start')
...
@admin_bp.route('/api/delete-official/<int:official_id>', methods=['POST'])
@admin_required
def delete_official(official_id):
    try:
        official = Official.query.get_or_404(official_id)
        official.is_active = False  # Soft delete
        official.updated_at = datetime.utcnow()
        db.session.commit()
        
        return jsonify({'status': 'success', 'message': 'Official removed successfully'})
    except Exception as e:
        db.session.rollback()
        return jsonify({'status': 'error', 'message': str(e)}), 400

@admin_bp.route('/api/get-official/<int:official_id>')
@admin_required
...
@admin_bp.route('/api/get-resident/<int:resident_id>')
@admin_required
...
@admin_bp.route('/api/add-resident', methods=['POST'])
@admin_required
def add_resident():
    """Add new resident"""
    try:
...
@admin_bp.route('/api/update-resident/<int:resident_id>', methods=['POST'])
@admin_required
def update_resident(resident_id):
    """Update existing resident and associated user account"""
    try:
        resident = Resident.query.get_or_404(resident_id)
        data = request.get_json()
        ...
        if resident.user_id:
            ...
            if user:
                # Update user's name to match resident's name
                user.name = resident.full_name
...
@admin_bp.route('/api/delete-resident/<int:resident_id>', methods=['POST'])
@admin_required
...
@admin_bp.route('/api/archived-residents')
@admin_required
...
@admin_bp.route('/api/restore-resident/<int:resident_id>', methods=['POST'])
@admin_required
...
@admin_bp.route('/api/get-certificate/<int:certificate_id>')
@admin_required
...
@admin_bp.route('/api/add-certificate', methods=['POST'])
@admin_required
def add_certificate():
...
@admin_bp.route('/api/update-certificate/<int:certificate_id>', methods=['POST'])
@admin_required
...
@admin_bp.route('/api/update-certificate-status/<int:certificate_id>', methods=['POST'])
@admin_required
def update_certificate_status(certificate_id):
    ...
    try:
        ...
        if old_status != 'completed' and new_status == 'completed':
            certificate.claimed_date = datetime.utcnow()
            certificate.completed_date = datetime.utcnow()
            
            # Debug logging
            print(f"DEBUG - Certificate {certificate_id} marked as completed")
            print(f"  Payment Status: {certificate.payment_status}")
            print(f"  Fee: {certificate.fee}")
            
            # If payment is already paid, automatically create payment record for payment list
            if certificate.payment_status == 'paid':
                print(f"  Payment is PAID - Creating payment record...")
                ...
                if not existing_payment:
                    try:
                        ...
                        payment = Payment(
                            ...
                            payment_status='paid',
                            payment_date=certificate.payment_date or datetime.utcnow(),
                        ...
                        db.session.add(payment)
                        print(f"  Payment record created successfully! Receipt: {payment.receipt_number}")
                        ...
                        try:
                            ...
                            pass  # Fields don't exist in database yet
                            
                    except Exception as payment_error:
                        print(f"ERROR creating payment: {payment_error}")
                        import traceback
                        traceback.print_exc()
                else:
                    print(f"  Payment record already exists - skipping creation")
            else:
                print(f"  Payment status is NOT paid ({certificate.payment_status}) - skipping payment record creation")
        
        db.session.commit()
        
        # Check if payment was created
        payment_list_message = ''
        if new_status == 'completed' and certificate.payment_status == 'paid':
            existing_payment = Payment.query.filter_by(certificate_id=certificate_id).first()
            if existing_payment:
                payment_list_message = ' and added to payment list'
        
        return jsonify({
            'status': 'success',
            'success': True,
            'message': f'Certificate status updated to {new_status}{payment_list_message}'
        })
...
@admin_bp.route('/api/update-payment-status/<int:certificate_id>', methods=['POST'])
@admin_required
def update_payment_status(certificate_id):
    ...
    try:
        ...
        if old_payment_status != 'paid' and new_payment_status == 'paid':
            ...
            # If certificate is already completed, automatically create payment record for payment list
...
@admin_bp.route('/api/delete-certificate/<int:certificate_id>', methods=['POST'])
@admin_required
...
@admin_bp.route('/api/get-residents-for-certificate')
@admin_required
...
@admin_bp.route('/api/blotter', methods=['GET'])
@admin_required
def get_blotters():
...
@admin_bp.route('/api/blotter/<int:blotter_id>', methods=['GET'])
@admin_required
...
@admin_bp.route('/api/blotter', methods=['POST'])
@admin_required
def create_blotter():
...
@admin_bp.route('/api/blotter/<int:blotter_id>', methods=['PUT'])
@admin_required
...
@admin_bp.route('/api/blotter/<int:blotter_id>', methods=['DELETE'])
@admin_required
...
@admin_bp.route('/blotter/<int:blotter_id>/print')
@admin_required
def print_single_blotter(blotter_id):
    ...
    try:
        ...
        return render_template('admin/print-blotter-single.html',
                             blotter=blotter,
                             barangay_info=barangay_info,
                             current_datetime=current_datetime)
    except Exception as e:
        flash(f'Error loading blotter for printing: {str(e)}', 'error')
        return redirect(url_for('admin.blotter'))

@admin_bp.route('/blotter/print')
@admin_required
def print_blotter_report():
    ...
    try:
        ...
        return render_template('admin/print-blotter-report.html',
                             blotters=blotters,
                             barangay_info=barangay_info,
...
@admin_bp.route('/certificate/<int:certificate_id>/payment')
@admin_required
def certificate_payment(certificate_id):
    ...
    try:
        ...
        return render_template('admin/certificate-payment.html', certificate=certificate)
        
    except Exception as e:
        flash(f'Error loading payment page: {str(e)}', 'error')
        return redirect(url_for('admin.certificates'))

@admin_bp.route('/api/certificate/<int:certificate_id>/reject', methods=['POST'])
@admin_required
...
@admin_bp.route('/api/certificate/<int:certificate_id>', methods=['GET'])
@admin_required
...
@admin_bp.route('/api/certificate/<int:certificate_id>', methods=['PUT'])
@admin_required
...
@admin_bp.route('/api/certificate/<int:certificate_id>/complete', methods=['POST'])
@admin_required
def complete_certificate_api(certificate_id):
    """Mark certificate as completed and create payment record if paid"""
    try:
        certificate = Certificate.query.get_or_404(certificate_id)
        ...
        # Debug logging
        print(f"DEBUG - complete_certificate_api - Certificate {certificate_id}")
        print(f"  Status changed to: completed")
        print(f"  Payment Status: {certificate.payment_status}")
        print(f"  Fee: {certificate.fee}")
        
        # If certificate is paid, automatically create/update payment record for payment list
        if certificate.payment_status == 'paid':
            print(f"  Payment is PAID - Will create payment record")
            
            # Check if payment record already exists
            existing_payment = Payment.query.filter_by(certificate_id=certificate_id).first()
            print(f"  Existing payment check: {existing_payment}")
            ...
            if not existing_payment:
                ...
                try:
                    ...
                    db.session.add(payment)
                    print(f"  Payment record added to session. Receipt: {payment.receipt_number}")
                ...
                print(f"  Payment record already exists - skipping")
        else:
            print(f"  Payment status is NOT paid ({certificate.payment_status}) - skipping")
        
        db.session.commit()
        
        return jsonify({
            'success': True,
            'message': 'Certificate marked as completed' + (' and added to payment list' if certificate.payment_status == 'paid' else ''),
            'certificate': certificate.to_dict()
        })
...
@admin_bp.route('/api/certificate/<int:certificate_id>/payment', methods=['PUT'])
@admin_required
...
@admin_bp.route('/api/certificate/<int:certificate_id>/move-to-payment-list', methods=['POST'])
@admin_required
...
@admin_bp.route('/api/sync-payment-list', methods=['POST'])
@admin_required
...
@admin_bp.route('/api/residents', methods=['GET'])
@admin_required
...
@admin_bp.route('/api/certificate-types', methods=['GET'])
@admin_required
...
@admin_bp.route('/api/certificate-types', methods=['POST'])
@admin_required
...
@admin_bp.route('/api/certificate-types/<int:type_id>', methods=['PUT'])
@admin_required
...
@admin_bp.route('/api/certificate-types/<int:type_id>', methods=['DELETE'])
@admin_required
...
@admin_bp.route('/api/certificate-types/sync', methods=['POST'])
@admin_required
...
@admin_bp.route('/api/barangay-info', methods=['GET'])
@admin_required
...
@admin_bp.route('/api/barangay-info', methods=['POST'])
@admin_required
...
@admin_required
def get_users():
    """Get all users excluding pending residents"""
    try:
        # Get all users ordered by role priority (admin/clerk first, then residents)
        ...
        for user in all_users:
            # Skip residents with pending status
...
@admin_bp.route('/api/purok', methods=['GET'])
@admin_required
def get_purok_list():
    """Get list of all purok/sitio with dynamic resident counts"""
    try:
        puroks = PurokInfo.query.filter_by(is_active=True).order_by(PurokInfo.name).all()
        purok_data = []
        
        for purok in puroks:
            # Get actual resident count from database
            resident_count = Resident.query.filter_by(
                sitio_id=purok.id,
                status='approved'
            ).count()
            
            # Convert to dict and update with actual count
            purok_dict = purok.to_dict()
            purok_dict['population_count'] = resident_count
            purok_dict['resident_count'] = resident_count
            
            purok_data.append(purok_dict)
...
@admin_bp.route('/api/purok/<int:purok_id>', methods=['GET'])
@admin_required
def get_purok(purok_id):
    """Get specific purok/sitio by ID with dynamic resident count"""
    try:
...
@admin_bp.route('/api/purok', methods=['POST'])
@admin_required
def create_purok():
...
@admin_bp.route('/api/purok/<int:purok_id>', methods=['PUT'])
@admin_required
...
@admin_bp.route('/api/purok/<int:purok_id>', methods=['DELETE'])
@admin_required
...
@admin_bp.route('/api/purok/<int:purok_id>/toggle-status', methods=['POST'])
@admin_required
...
@admin_bp.route('/api/purok/sync-counts', methods=['POST'])
@admin_required
def sync_purok_counts():
    """Sync all purok/sitio population counts with actual resident counts"""
    try:
        puroks = PurokInfo.query.all()
        updated_count = 0
        ...
        for purok in puroks:
            ...
            # Update the population_count field
            if purok.population_count != resident_count:
                purok.population_count = resident_count
                updated_count += 1
        ...
        return jsonify({
            'success': True,
            'message': f'Successfully synced resident counts. Updated {updated_count} out of {len(puroks)} purok/sitio records.',
            'updated_count': updated_count,
            'total_count': len(puroks)
        })
        ...
        return jsonify({
            'success': False,
            'message': f'Error syncing purok counts: {str(e)}'
        }), 500

@admin_bp.route('/api/revenue-analytics')
@admin_required
def revenue_analytics_api():
    """API endpoint to get revenue analytics data for charts"""
    try:
        ...
        for i in range(5, -1, -1):  # Last 6 months
            # Calculate the target month and year more accurately
            if today.month - i <= 0:
            ...
            # Calculate revenue ONLY from payments table (not certificates)
            payment_revenue = db.session.query(func.sum(Payment.amount)).filter(
                Payment.payment_status == 'paid',
            ...
            # Total revenue for the month (only from payments)
            total_revenue = float(payment_revenue)
            months_data.append(total_revenue)
            
            # Debug info for this month
            debug_info.append({
                'month': month_name,
                'month_num': month,
                'year': year,
                'payment_revenue': float(payment_revenue),
        ...
        if len(months_data) >= 2 and months_data[-2] > 0:
            revenue_change = round(((months_data[-1] - months_data[-2]) / months_data[-2]) * 100, 1)
        
        # Get all payments for debugging
        all_payments = Payment.query.filter_by(payment_status='paid').all()
        payment_debug = []
        for payment in all_payments:
            payment_debug.append({
                'id': payment.id,
                'amount': float(payment.amount),
                'payment_date': payment.payment_date.strftime('%Y-%m-%d %H:%M:%S') if payment.payment_date else 'No date',
                'payment_month': payment.payment_date.month if payment.payment_date else 'No month',
                'payment_year': payment.payment_date.year if payment.payment_date else 'No year',
                'payment_status': payment.payment_status,
                'service_type': payment.service_type
            })
        ...
        return jsonify({
            ...
            'data': {
                ...
                'revenue': months_data,
                'revenue_change': revenue_change,
                'current_month_revenue': months_data[-1] if months_data else 0,
                'total_revenue': sum(months_data)
            },
            'debug': {
                'months_breakdown': debug_info,
                'total_paid_payments': len(all_payments),
                'payments_data': payment_debug,
                'current_month': today.month,
                'current_year': today.year,
                'note': 'Revenue calculated from payments table only',
                'chart_months_calculated': [(today.month - i if today.month - i > 0 else 12 + (today.month - i), today.year if today.month - i > 0 else today.year - 1) for i in range(5, -1, -1)]
            }
        })
